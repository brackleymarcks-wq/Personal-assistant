    ...
    grid.innerHTML = `
      ...
      <div class="bento-item bento-col-8" style="display:flex;flex-direction:column;">
        ...
        ${todayTasks.length === 0 && overdueTasks.length === 0 ? `<div style="color:var(--text-muted);font-size:14px;">Всё выполнено! 🎉</div>` : `
          <div style="display:flex; flex-wrap:wrap; gap:12px; width:100%; flex:1; align-content:flex-start;">
            ${[...overdueTasks, ...todayTasks.filter(t => !overdueTasks.includes(t))].slice(0, 6).map((t) => {
              ...
              return `
              <div style="flex: 1 1 calc(40% - 12px); min-width: 200px; display:flex; align-items:center; gap:12px; padding:16px; border-radius:12px; background:${sc}0f; border:1px solid ${sc}40; cursor:pointer; transition:all 0.2s ease; min-height:54px; box-sizing:border-box; overflow:hidden;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'" onclick="App.navigateTo('tasks')">
                ...
                <span style="font-size:14px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text-primary);font-weight:500;min-width:0;">${this.esc(t.title)}</span>
              </div>
              `;
            }).join('')}
          </div>
